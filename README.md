...
Copy [.env.example](.env.example) to **`.env.local`** (or `.env`). It matches the live stack: **`NEXT_PUBLIC_API_BASE_URL`** → [`https://quest-room-api.vercel.app`](https://quest-room-api.vercel.app), **`NEXT_PUBLIC_APP_URL`** → [`https://quest-room-nextjs-frontend.vercel.app`](https://quest-room-nextjs-frontend.vercel.app). Set **`NEXT_PUBLIC_GOOGLE_MAPS_API_KEY`** if you use the contacts map. For a fully local run, change those two to your local API and app URLs.

## Environment variables

| Variable                          | Where it runs                       | Description                                                                                                                                                                                                                  |
| --------------------------------- | ----------------------------------- | ---------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| `NEXT_PUBLIC_API_BASE_URL`        | Server + Client                     | REST API base URL **without** a trailing `/`. Production: `https://quest-room-api.vercel.app`. Used by Server Actions today; the `NEXT_PUBLIC_` prefix keeps the value available in both server and client bundles.                              |
...
On **Vercel** → **Settings → Environment Variables**, set `NEXT_PUBLIC_API_BASE_URL`, `NEXT_PUBLIC_APP_URL`, and optionally `NEXT_PUBLIC_GOOGLE_MAPS_API_KEY` for **Production** (and Preview if you use preview deployments).
...
You need at least `NEXT_PUBLIC_API_BASE_URL` and `NEXT_PUBLIC_APP_URL` for `build` / `start` (or use `.env.local`).
...
| Role                               | Repository                                                                                                                                                                                                                                      |
| ---------------------------------- | ----------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- |
| **API (NestJS)**                   | [Vitalina-Yefimova/quest-room-api](https://github.com/Vitalina-Yefimova/quest-room-api) · Live: [quest-room-api.vercel.app](https://quest-room-api.vercel.app)                                                                                  |
...
**`NEXT_PUBLIC_API_BASE_URL`** and **`NEXT_PUBLIC_APP_URL`** must match the API and the URL people use to open this app (the values in [`.env.example`](.env.example) for the current Vercel deploy) so email verification and password-reset links work.
...
- HTTP: Server Actions use `fetch` with `NEXT_PUBLIC_API_BASE_URL` from [`src/utils/config.ts`](src/utils/config.ts) (for example `${NEXT_PUBLIC_API_BASE_URL}/quests`).
